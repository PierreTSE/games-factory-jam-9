...
void SoundWave::update()
{
    ...
    if(radius_ < 2400)
    {
        if(alpha_ != 0)
        {
            ...
            wave_.setOutlineColor({255, 255, 255, static_cast<sf::Uint8>(alpha_)});
...
void SoundWave::setRadiusSpeed(float sp) { radiusSpeed_ = sp; }

void SoundWave::setAlphaSpeed(float sp) { alphaSpeed_ = sp; }

void SoundWave::setRadius(float r) { radius_ = r; }

void SoundWave::setAlpha(float a) { alpha_ = a; }
...
bool SoundWave::isDead() { return echo_.isDead(); }

void SoundWave::setDrawCircle(bool b) { drawCircle = b; }

sf::Time SoundWave::getTime() { return timer_; }
